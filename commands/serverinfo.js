...
module.exports.run = async (client, message, args, utils) => {
	...
	const embed = new MessageEmbed()
		...
		.addField('General', [
			...
			`**<:crown:785125378187460618> Owner:** ${message.guild.owner.user.tag} (${message.guild.ownerID})`,
			`** :map: Region:** ${regions[message.guild.region]}`,
			`** <785122045951344701> Boost Tier:** ${message.guild.premiumTier ? `Tier ${message.guild.premiumTier}` : 'None'}`,
			`** <785126074619133982> Explicit Filter:** ${filterLevels[message.guild.explicitContentFilter]}`,
			`**<a785125902023524353> Verification Level:** ${verificationLevels[message.guild.verificationLevel]}`,
			'\u200b',
		])
		.addField('Server', [
